...
fun HomeScreen(onNavigateToChat: () -> Unit) {
    ...
    var apiKey by remember {
        val saved = prefs.getString("api_key", "")?.trim() ?: ""
        if (saved.isNotEmpty() && saved != "MY_GEMINI_API_KEY" && saved != "YOUR_API_KEY") {
            mutableStateOf(saved)
        } else {
            val buildKey = try { com.example.BuildConfig.GEMINI_API_KEY.trim() } catch (e: Exception) { "" }
